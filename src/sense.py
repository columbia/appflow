...
last_screen_name = "appmodelLast" + util.randstr(4) + ".png"
...
def grab_hier_xml(dev):
    tempf = tempfile.mktemp(prefix=prefix, suffix='.xml', dir=config.tempdir)
    has_something = False
    content = None
...
def grab_screen_cap(dev, tempf):
    dev.remove_file("/sdcard/screen.raw")
    logger.info("grabbing into %s", tempf)
    if config.use_old_capture_method:
        dev.run_adb_shell("screencap -p /sdcard/screen.png")
        dev.grab_file("/sdcard/screen.png", tempf)
    else:
        dev.run_adb_shell("screencap /sdcard/screen.raw")
        rawtempf = tempfile.mktemp(suffix=".raw", prefix=prefix, dir=config.tempdir)
        rgbatempf = tempfile.mktemp(suffix=".rgba", prefix=prefix, dir=config.tempdir)
        dev.grab_file("/sdcard/screen.raw", rawtempf)
        subprocess.check_call("../scripts/scrshot.sh %s %s %s" % (rawtempf, rgbatempf,
                                                                  tempf), shell=True)
...
def grab_screen(dev):
    tempf = tempfile.mktemp(suffix=".png", prefix=prefix, dir=config.tempdir)
...
def grab_screen_fast(dev, windowid=None):
    ...
    try:
        tempf = tempfile.mktemp(suffix=".bmp", prefix=prefix, dir=config.tempdir)
...
def same_snapshot(old, new):
    return compare_snapshot(old, new) == 0


@perfmon.op("sense", "same_snapshot")
def compare_snapshot(old, new, fast=False):
...
def wait_idle(dev, interval=0.2, count=3, maxtry=None):
    ...
    for i in range(maxtry):
        ...
        if snapshot is not None:
            pixdiff = compare_snapshot(snapshot, new_snapshot)
            if pixdiff == 0:
                same_cnt += 1
            else:
                same_cnt = 0
...
def round_clear():
    logger.info("clearing %s/%s*" % (config.tempdir, prefix))
    subprocess.call("rm -f %s/%s*" % (config.tempdir, prefix), shell=True)
...
def dump_page(dev, path, appname=None, scrname=None, guispath=None):
    ...
    filename = os.path.join(path, template % page_id)
    dump_to_file(dev, filename, appname, scrname, guispath)
...
def duplicate_screen(screen):
    dup_screen = copy.deepcopy(screen)
    last_screen_file = os.path.join(config.tempdir, last_screen_name)
    if 'scr' in screen:
        shutil.copy(screen['scr'], last_screen_file)
        dup_screen['oldscr'] = screen['scr']
        dup_screen['scr'] = last_screen_file
    return dup_screen


def restore_screen(screen):
    dup_screen = copy.deepcopy(screen)
    if 'oldscr' in dup_screen:
        shutil.copy(screen['scr'], screen['oldscr'])
        dup_screen['scr'] = dup_screen['oldscr']
    return dup_screen
